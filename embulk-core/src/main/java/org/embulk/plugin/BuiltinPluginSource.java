    ...
    public <T> T newPlugin(final Class<T> pluginInterface, final PluginType type) throws PluginSourceNotMatchException {
        ...
        if (InputPlugin.class.isAssignableFrom(pluginInterface)) {
            ...
            if (fileInputPluginImpl != null) {
                return pluginInterface.cast(new FileInputRunner((FileInputPlugin) this.injector.getInstance(inputPluginImpl)));
            }
            ...
            if (fileOutputPluginImpl != null) {
                return pluginInterface.cast(new FileOutputRunner((FileOutputPlugin) this.injector.getInstance(outputPluginImpl)));
            }
        } else {
            final Class<?> impl;
